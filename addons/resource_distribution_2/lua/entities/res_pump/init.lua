...
function ENT:ResetPlug()
	if (self.DeployedPlug == 1) then
		...
		elseif (self.DeployedPlug == 1 and not self.plug:IsValid()) then
			self.ropelength = 0
			self.ropemax = 0
			self.Hose = nil
			self.rope = nil
...
function ENT:Think()
	...
	if (self.Weld and not self.Weld:IsValid()) or (self.MyPlug and not self.MyPlug:IsValid()) or (self.OtherSocket and not self.OtherSocket:IsValid()) or (self.plug and not self.plug:IsValid()) then
	...
	elseif (self.reel_status > REEL_STOP) then --plug deployed and we need to do something with the reel
		...
		if (self.reel_status == REEL_OUT) then
			local dist = (self.Entity:GetPos() - self.plug:GetPos()):Length()
...
function ENT:ReelInPlug()
	if not (self.DeployedPlug == 1) then return end
...
function ENT:EjectPlug()
	if not (self.Connected == 1) then return end
...
function ENT:Deploy()
	...
	local ang = self.Entity:GetAngles() + Vector(180,0,0):Angle()

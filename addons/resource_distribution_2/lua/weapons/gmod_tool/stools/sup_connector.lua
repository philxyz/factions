...
if ( CLIENT ) then
	...
	language.Add( 'tool.sup_connector.desc', 'Create a Supply Connector on any surface.' )
	language.Add( 'tool.sup_connector.0', 'Left-Click: Spawn a Supply Connector.' )
...
function TOOL:LeftClick( trace )
	...
	if ( not self:GetSWEP():CheckLimit( "connectors" ) ) then return false end
...
if (SERVER) then 
	...
	function MakeSupplyConnector( ply, Ang, Pos, pump, rate, hoselength )
		if ( not ply:CheckLimit( "connectors" ) ) then return nil end

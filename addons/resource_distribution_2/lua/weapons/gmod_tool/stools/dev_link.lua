...
if ( CLIENT ) then
	...
	language.Add( "tool.dev_link.0", "Left Click: Link two devices.  Right Click: Unlink two devices.  Reload: Unlink device from all others." )
    language.Add( "tool.dev_link.1", "Click on another resource-carrying device (or vehicle pod)." )
    language.Add( "tool.dev_link.2", "Right-click on another resource-carrying device (or the same one to unlink all others.)" )
    language.Add( "linktool.addlength", "Add Length:" )
    language.Add( "linktool.width", "Width:" )
    language.Add( "linktool.material", "Material:" )
end
...
function TOOL:LeftClick( trace )
	...
	if (SERVER and not trace.Entity.resources2) then
		self:ClearObjects()
		if ( iNum > 0 ) then
	...
	if ( iNum > 0 ) then
		...
		if (Ent1.is_valve) and (Ent2.is_valve) then
			self:ClearObjects()
	        self:GetOwner():SendLua( "GAMEMODE:AddNotify('A pair of valves can not be linked together.', NOTIFY_GENERIC, 7);" )
			return
		end
		...
		if (Ent1 == Ent2) or (Ent1.resources2links and Ent1.resources2links[ Ent2:EntIndex() ] and Ent2.resources2links and Ent2.resources2links[ Ent1:EntIndex() ]) then
			self:ClearObjects()
			self:GetOwner():SendLua( "GAMEMODE:AddNotify('These devices are already linked.', NOTIFY_GENERIC, 7);" )
			return
		end
		
		if Ent1:GetPos():Distance(Ent2:GetPos()) > RD_MAX_LINK_LENGTH then
			self:ClearObjects()
			self:GetOwner():SendLua( "GAMEMODE:AddNotify('Devices too far apart to be linked.', NOTIFY_GENERIC, 7);" )
			return
		end
...
function TOOL:Reload(trace)
	...
	if (not trace.Entity.resources2) then
		self:ClearObjects()
        self:GetOwner():SendLua( "GAMEMODE:AddNotify('Conduit invalid.', NOTIFY_GENERIC, 7);" )
		return
	end

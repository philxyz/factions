	...
	local emitter = ParticleEmitter( vOffset )
		...
		for i=0, (NumParticles) do
			...
			if (particle) then
				...
				particle:SetColor( Color(100 , 80 , 50) )
			end
		...
		for i=0, (NumParticles / 5) do
			...
			local particle = emitter:Add( "particles/smokey", vOffset + Pos * math.Rand(20, 500 ))
			if (particle) then
				particle:SetVelocity( Vector(0,0,1) * math.Rand(50, 100) )
				particle:SetLifeTime( 0 )
				particle:SetDieTime( math.Rand( 20, 30 ) )
				particle:SetStartAlpha( math.Rand( 200, 255 ) )
				particle:SetEndAlpha( 0 )
		...
		for i=0, (NumParticles) do
			...
			local particle = emitter:Add( "particles/flamelet"..math.random(1,5), vOffset + Pos * math.Rand(1000, 1500 ))
			if (particle) then

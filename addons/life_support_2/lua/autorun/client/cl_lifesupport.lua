...
hook.Add("PreDrawHalos", "AddLSHalos", function()
	local tr = util.GetPlayerTrace(LocalPlayer())
	local trRes = util.TraceLine(tr)
	for _,ent in pairs(ents.FindInSphere(LocalPlayer():GetPos(), 150)) do
		if ent.DrawHalo and trRes.Entity == ent then
			halo.Add({ent}, Color(255,255,255), 1, 1, 2)
			break
		end
	end
end)
